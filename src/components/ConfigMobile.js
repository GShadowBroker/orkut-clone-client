...
const Config = ({ configOpen, setConfigOpen, toggleConfig, loggedUser }) => {
  ...
  const [editProfile, { loading: loadingSubmission }] = useMutation(
    EDIT_PROFILE,
    {
      ...
      onCompleted: () => {
        history.push(`/perfil/${loggedUser.id}`);
        toggleConfig();
      },
    }
  );
  ...
  return (
    ...
    >
      <ConfigForm onSubmit={handleSubmit(onSubmit)}>
        {errorMessage && (
          <Notification message={errorMessage} style={{ margin: 0 }} />
        )}
        ...
        {errors.interests && errors.interests.type === "maxLength" && (
          <ErrorBoxContainer>
            ...
            <span>
              {" "}
              O campo de interesses não pode ultrapassar 100 caractéres
            </span>
          </ErrorBoxContainer>
        )}
        ...
        <ButtonGroup>
          <Button type="submit">
            <strong>{loadingSubmission ? "salvando..." : "salvar"}</strong>
          </Button>
