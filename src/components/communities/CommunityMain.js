        ...
        <MainColumn>
            ...
            <Card>
                <ProfileInfo>
                            ...
                            viewFullProfile ? (
                                <div style={{
                                    ...
                                    <div style={{marginLeft: '.5rem', minWidth: '46%'}}>
                                        <p><strong>idioma: </strong>{ community.language }</p>
                                        <p><strong>categoria: </strong>{ community.Category.title }</p>
                                        <p><strong>tipo: </strong>{ community.type }</p>
                                        <p><strong>visível por: </strong>todos</p>
                                    </div>
                                    <div style={{marginLeft: '.5rem', minWidth: '46%'}}>
                                        <p><strong>criada em: </strong>{ new Date(community.createdAt).toLocaleString('pt-BR', timeOptions) }</p>
                                        <p><strong>local: </strong>{ community.country }</p>
                                        <p>
                                            <strong>proprietário: </strong>
                                            <Link to={`/perfil/${community.Creator.id}`}>{ community.Creator.name }</Link>
                                        </p>
                    ...
                    <ProfileSection>
                        <div>
                            <p style={{ lineHeight: '1.6' }}>{ community.description ? community.description : 'Sem descrição.' }</p>
                        </div>
                    </ProfileSection>
                </ProfileInfo>
            </Card>
            ...
            <Card style={{ marginTop: '.6em' }}>
                                    ...
                                    <Message style={{borderBottom: '.3px solid #e3e8f5'}}>
                                        ...
                                        <MessageContent>
                                            <MessageHeaderSpaced style={{fontSize: '1.1em'}}>
                                                <FakeLink><strong>{ trunc(t.title, 45) }</strong></FakeLink>
                                            ...
                                            { t.Comments[0]
                                                ? (<MessageBody>
                                                    <MessageDetails>
                                                        ...
                                                        <span>Última postagem: { trunc(t.Comments[0].body.replace(/<[^>]*>/g, ''), 25) } </span>
                                                ...
                                                : (<MessageBody>
                                                        <MessageDetails>
                                                            ...
                                                            <span>Última postagem: { trunc(t.body.replace(/<[^>]*>/g, ''), 25) } </span>
                                                            <span style={{ fontSize: '.9em' }}>
                ...
                { topics.length > 0
                    ? (<ProfileInfo style={{paddingBottom: '1.2rem'}}>
                        <Link to={`/comunidades/${community.id}/forum`}>ver todos os tópicos</Link>
                    </ProfileInfo>)
                    : <ProfileInfo style={{paddingBottom: '1.2rem', color: 'grey'}}><span>Nenhum tópico</span></ProfileInfo>
                }
            </Card>
        </MainColumn>
    )
