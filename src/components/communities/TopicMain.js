...
const TopicMain = ({ user, community, topics, mobile }) => {
  ...
  const [sendComment, { loading: loadingCommentSubmission }] = useMutation(
    SEND_COMMENT,
    {
  ...
  const [removeComment, { loading: loadingCommentRemoval }] = useMutation(
    REMOVE_COMMENT,
    {
      onError: (error) => errorHandler(error, setErrors),
      refetchQueries: [
  ...
  const [removeTopic, { loading: loadingTopicRemoval }] = useMutation(
    REMOVE_TOPIC,
    {
      onError: (error) => errorHandler(error, setErrors),
      refetchQueries: [
              ...
              <div style={{ maxWidth: "80%" }}>
                <Link to={`/comunidades/${communityId}/forum/${topicId}`}>
                  <Subtitle>
                    ...
                    >
                      {commentCount === 1
                        ? `- ${commentCount} resposta`
                        : `- ${commentCount} respostas`}
                    </span>
                  </Subtitle>
                </Link>
              </div>
          ...
          <Message style={{ borderTop: ".3px solid #e3e8f5" }}>
            ...
            <MessageContent>
              <CommentSectionHeader style={{ margin: 0 }}>
                <div>
                  <Link to={`/perfil/${topic.TopicCreator.id}`}>
                    <strong>{trunc(topic.TopicCreator.name, 50)}</strong>
                  </Link>
                  <Time>
                    -{" "}
                ...
                {user.id === topic.TopicCreator.id && comments.length === 0 ? (
                  <MessageActions>
                    {loadingTopicRemoval ? (
                      ...
                      <FakeLink onClick={() => handleTopicDelete(topic)}>
                        excluir tópico
                      </FakeLink>
                    )}
                  </MessageActions>
            ...
            >
              ...
              <MessageContent>
                <CommentSectionHeader style={{ margin: 0 }}>
                  <div>
                    <Link to={`/perfil/${c.Sender.id}`}>
                      <strong>{trunc(c.Sender.name, 50)}</strong>
                    </Link>
                    <Time>
                      -{" "}
                  ...
                  {user.id === c.Sender.id && (
                    <MessageActions>
                      {loadingCommentRemoval ? (
                        ...
                        <FakeLink onClick={() => handleCommentDelete(c)}>
                          excluir
                        </FakeLink>
                      )}
                    </MessageActions>
