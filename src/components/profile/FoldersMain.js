    ...
    <MainColumn stretched>
      ...
      >
        <Form onSubmit={handleSubmit(onSubmit)}>
          ...
          {formErrors.visible_to_all &&
          formErrors.visible_to_all.type === "required" ? (
            <ErrorBoxContainer>
              <RiErrorWarningLine className="icenter" />
          ...
          <ModalActionGroup>
            <Button type="submit" disabled={loadingAlbumCreation}>
              <strong>
                {loadingAlbumCreation ? "criando..." : "criar álbum"}
              </strong>
            </Button>
      ...
      <Card>
        ...
        <ProfileInfo>
          {albuns.length > 0 ? (
            <AlbumGrid>
              {albuns.map((a) => {
                if (a.visible_to_all || (!a.visible_to_all && meOrFriend))
                  return (
                    <AlbumContainer key={a.id}>
                      <Link
                        to={`${match.url}/${a.id}/fotos`}
            ...
            <AlbumGrid>
              <p style={{ color: "#afafaf" }}>Nenhum álbum</p>
            </AlbumGrid>
          )}
        </ProfileInfo>
      </Card>
