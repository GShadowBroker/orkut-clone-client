...
const HomeMain = ({
  ...
  useEffect(() => {
    ...
    if (savedConfig && savedConfig.lastRead) {
      if (
        new Date().getDate() > Number(savedConfig.lastRead) + 1 ||
        (new Date().getDate() === 1 && Number(savedConfig.lastRead) !== 1)
      ) {
        const fortune =
          todaysFortune[Math.floor(Math.random() * todaysFortune.length)];
        const config = {
          ...savedConfig,
    ...
    <MainColumn>
      ...
      <Card>
        <ProfileInfo style={{ marginTop: ".6rem" }}>
          ...
          {post.length >= 1000 && (
            <InputWarning>limite de caracteres atingido (1000)</InputWarning>
          )}
        </ProfileInfo>
        <ProfileInfo>
          <InlineHeader>
            ...
            >
              <Subtitle2 style={{ marginRight: ".3rem" }}>
                Atualizações de:
              </Subtitle2>
        ...
        {user.Requesters.map((u) => (
          <ProfileInfo key={u.id} style={{ margin: ".5rem 0" }} borderbottom>
            <Message>
              ...
              <MessageContent>
                <MessageHeader>
                  <Subtitle2 style={{ marginTop: 0 }}>
                    <strong>Novo pedido de amizade</strong>
                  </Subtitle2>
                </MessageHeader>
                <MessageBody>
                  <Link to={`/perfil/${u.id}`}>{trunc(u.name, 20)} </Link>quer
                  ser seu amigo no orkut.
                </MessageBody>
                <MessageActions below>
                  <Button onClick={() => handleRespondFriendRequest(u, true)}>
                    <strong>aceitar</strong>
                  </Button>
                  <Button onClick={() => handleRespondFriendRequest(u, false)}>
                    rejeitar
                  </Button>
                </MessageActions>
              </MessageContent>
          ...
          >
            <Message>
              ...
              <MessageContent>
                <MessageHeader>
                  <Link to={`/perfil/${f.User.id}`}>
                    {trunc(f.User.name, 30)}
                  </Link>
                  <Time>
                    -{" "}
                    {new Date(f.createdAt).toLocaleString("pt-BR", timeOptions)}{" "}
        ...
        {hasMoreFeed && (
          <ShowMore>
            ...
            >
              <TiArrowSortedDown className="icenter" /> ver mais
            </FakeLink>
          </ShowMore>
        )}
      </Card>
