...
const PreviewImageGrid = styled.div`
  ...
  grid-template-columns: ${(props) =>
    props.mobile ? "80px 80px 80px" : "80px 80px 80px 80px"};
  grid-gap: 0.5rem;
...
const PhotosMain = ({ user, loggedUser, albuns, mobile }) => {
  ...
  const [removeAlbum, { loading: loadingAlbumRemoval }] = useMutation(
    REMOVE_ALBUM,
    {
      onError: (error) => errorHandler(error, setErrors),
      onCompleted: () => history.push(`/perfil/${userId}/albuns`),
      refetchQueries: [{ query: GET_USER_ALBUNS, variables: { userId } }],
    }
  );
  const [uploadNewPhotos, { loading: loadingUpload }] = useMutation(
    UPLOAD_NEW_PHOTOS,
    {
      onError: (error) => errorHandler(error, setErrors),
      onCompleted: () => {
  ...
  const handleAlbumDelete = () => {
    const question = window.confirm(
      "Tem certeza de que deseja excluir este álbum e todo o seu conteúdo?"
    );
    if (!question) return;
    removeAlbum({
      variables: {
    ...
    <MainColumn stretched>
      ...
      >
        {preview.length > 0 && (
          <InlineHeader>
            <p>
              <strong>fotos adicionadas ({preview.length}):</strong>
            </p>
        ...
        <PreviewImageGrid mobile={mobile}>
          {preview.length > 0
            ? preview
                .map((i, index) => <Image key={index} url={i} size={80} />)
                .concat(
                  Array.from(
                    Array(8 - preview.length || 1).keys()
                  ).map((i, index) => <div key={index}></div>)
                )
        ...
        <Form onSubmit={onSubmit}>
          <ModalInputGroup>
            <label htmlFor="newphoto">
              <strong>selecione uma foto ou mais:</strong>
            </label>
            <Input
              id="newphoto"
              type="file"
        ...
        {loadingPhotos ? (
          ...
          <ProfileInfo>
            <PhotoList>
              ...
              {photos.length === 0 && (
                <span style={{ color: "grey" }}>Nenhuma foto</span>
              )}
            </PhotoList>
            <ShowMore>
