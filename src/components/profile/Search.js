...
const Search = ({ loggedUser }) => {
  ...
  const [
    getAllUsers,
    { loading: loadingNewUsers, data: dataNewUsers },
  ] = useLazyQuery(GET_ALL_USERS);
  const [
    getAllCommunities,
    { loading: loadingNewCommunities, data: dataNewCommunities },
  ] = useLazyQuery(GET_ALL_COMMUNITIES);
  ...
  if (dataNewUsers && dataNewUsers.allUsers) {
    ...
    communityCount =
      dataNewCommunities && dataNewCommunities.allCommunities.count;
  }
      ...
      <Card>
        <ProfileInfo>
          <InlineHeader style={{ paddingBottom: "0" }}>
            <Subtitle>
              Termo pesquisado: {decodeURI(search) || newSearch}
            </Subtitle>
          </InlineHeader>
        ...
        {communities.map((community) => (
          <Message>
            ...
            <MessageContent>
              <MessageHeaderSpaced>
                <Link to={`/comunidades/${community.id}`}>
                  <strong>{trunc(community.title, 30)}</strong>
                </Link>

                {community.Comments[0] && (
                  <Time>
                    {new Date(community.Comments[0].createdAt).toLocaleString(
                      "pt",
                      timeOptions
                    )}{" "}
                    ({moment(community.Comments[0].createdAt).fromNow()})
                  </Time>
                )}
              </MessageHeaderSpaced>
              {community.Topics.length > 0 ? (
                <MessageBody>
                  <p style={{ marginBottom: ".3rem" }}>
                    Última postagem:{" "}
                    {new Date(community.Topics[0].createdAt).toLocaleDateString(
                      "pt",
                      timeOptions
                    )}{" "}
                    ({moment(community.Topics[0].createdAt).fromNow()})
                  </p>
                  <Link
                    to={`/comunidades/${community.id}/forum/${community.Topics[0].id}`}
                  >
                    {trunc(community.Topics[0].title, 30)}
                  </Link>
                </MessageBody>
              ) : (
                <MessageBody>
