...
import { ProfileInfo, InlineHeader, MainColumn } from "../../styles/profile";
...
const Testimonials = ({ user, loggedUser, mobile }) => {
  ...
  const [
    removeTestimonial,
    { loading: loadingTestimonialRemoval },
  ] = useMutation(REMOVE_TESTIMONIAL, {
    onError: (error) =>
      error.graphQLErrors
        ? alert(error.graphQLErrors[0].message)
        : alert("Erro de servidor"),
  ...
  return (
    <MainColumn stretched>
      ...
      <Card style={{ paddingBottom: ".6rem" }}>
        <ProfileInfo>
          <InlineHeader>
            <Subtitle>
              <strong>
                {user.id === loggedUser.id ? "Meus Depoimentos" : "Depoimentos"}{" "}
            ...
            <Message key={t.id}>
              ...
              <MessageContent>
                <MessageHeader>
                  ...
                  <Time>
                    -{" "}
                    {new Date(t.createdAt).toLocaleString("pt-BR", timeOptions)}{" "}
              ...
              {(user.id === loggedUser.id || t.Sender.id === loggedUser.id) && (
                <MessageActions>
                  {loadingTestimonialRemoval ? (
                    ...
                    <FakeLink onClick={() => handleTestimonialRemoval(t)}>
                      excluir
                    </FakeLink>
                  )}
                </MessageActions>
