...
const CommunityRoute = ({ loggedUser }) => {
  ...
  const {
    ...
    data: dataTopics,
  } = useQuery(FETCH_TOPICS, {
    variables: { communityId, limit: 5, offset: 0, limitComment: 1 },
  });
  const [joinCommunity, { loading: loadingJoin }] = useMutation(
    JOIN_COMMUNITY,
    {
  ...
  const [leaveCommunity, { loading: loadingLeave }] = useMutation(
    LEAVE_COMMUNITY,
    {
      onError: (error) => errorHandler(error, setErrors),
      refetchQueries: [
  ...
  if (loading || loadingMembers || loadingTopics)
    return (
      <Main>
  ...
  if (error || errorMembers || errorTopics || errors)
    return <Notification message={errors ? errors : null} />;
  ...
  const handleJoinCommunity = () => {
    const answer = window.confirm(
      `Deseja participar da comunidade ${community.title}?`
    );
    if (!answer) return;
    joinCommunity({
      variables: {
  ...
  const handleLeaveCommunity = () => {
    const answer = window.confirm(
      `Deseja mesmo sair da comunidade ${community.title}?`
    );
    if (!answer) return;
    leaveCommunity({
      variables: {
    ...
    <Main>
      ...
      <Switch>
        <Route exact path={`${match.path}/forum/:topicId`}>
          <TopicMain user={loggedUser} community={community} topics={topics} />
        </Route>
