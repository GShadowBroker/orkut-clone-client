...
const ProfilePhotos = ({ loggedUser, mobile }) => {
  ...
  const {
    ...
    data: dataAlbuns,
  } = useQuery(GET_USER_ALBUNS, {
    variables: { userId },
  });
  const [createNewAlbum, { loading: loadingAlbumCreation }] = useMutation(
    CREATE_NEW_ALBUM,
    {
      ...
      onCompleted: (data) => {
        ...
        handleModal();
        history.push(
          `/perfil/${userId}/albuns/${data.createPhotoFolder.id}/fotos`
        );
      },
    }
  );
  ...
  const getAlbumCount = () => {
    ...
    return (
      albuns.length - albuns.filter((a) => a.visible_to_all === false).length
    );
  };
